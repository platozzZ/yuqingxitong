		...
		<view class="padding-top-sm">
			...
			<view class="topic-content  margin-bottom-sm" v-if="tabType == 'p'">
				<view class="" v-for="(item,index) in topicData" :key="'p' + index">
					...
					<view class="cu-bar bg-white solid-bottom text-red text-bold margin-top-sm flex justify-between align-center">
					...
					<view class="cu-list menu sm-border">
						<view class="cu-item" v-for="(iteml,indexl) in item.mediaList" :key="'p' + index + indexl" v-if="indexl<10">
							<view class="content">
								<view class="response">
									<text class="text-cut">{{iteml.Title}}</text>
								</view>
							</view>
							<view class="action text-right text-red">
								{{iteml.Article_ID}}
							</view>
						</view>
					</view>	
			...
			<view class="topic-content margin-sm" v-if="tabType == 'u'">
				<view class="" v-for="(item,index) in topicData" :key="'u' + index">
					<view class="text-red text-xl margin-top margin-bottom-sm text-center">
						{{item.type == 'all'? $config.brand + '全车型新闻舆情重点话题TOP10':item.type + '新闻舆情重点话题TOP10'}}
					</view>
					<view class="coop-title bg-red flex justify-around align-center text-xs text-center">
					...
					<view class="coop-list flex justify-around align-center text-xs text-red text-center solid-left" v-for="(iteml,indexl) in item.mediaList" :key="'u' + index + indexl">
					...
					<!-- <view class="cu-list menu sm-border">
						<view class="cu-item" v-for="(iteml,indexl) in item.mediaList" :key="iteml.Article_ID" v-if="indexl<10">
							<view class="content padding-tb-xs">
								<view class="response">
									<text class="text-cut">{{iteml.Title}}</text>
								</view>
								<view class="flex justify-end align-center text-sm text-gray">
									<text class="cuIcon-attention padding-right-xs"></text>{{iteml.Article_ID}}
									<text class="padding-lr-sm">|</text>
									<text class="cuIcon-mark padding-right-xs"></text>{{iteml.Article_ID}}
									<text class="padding-lr-sm">|</text>
									来源：{{iteml.Article_ID}} 
								</view>
							</view>
