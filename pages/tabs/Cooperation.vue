		...
		<view class="padding-top-sm">
			<view class=" flex justify-center">
				<view class="main-capsule text-xs flex justify-center text-df">
					<view class="main-capsule-item flex align-center" :class="index==TabCur?' cur':''" v-for="(item,index) in tabList" :key="index" @tap.stop="tabSelect" :data-id="index" :data-target="item.id" :data-type="item.type">
						{{item.name}} 
	...
	export default {
		...
		methods: {
			...
			slotBtnClick(){
				...
				that.getMediaList({sWhereSQL:datas})
				// that.slotDisplay = 'none'
				that.showSlotContent = false
			},
			formatNumber(n){
			  n = n.toString()
